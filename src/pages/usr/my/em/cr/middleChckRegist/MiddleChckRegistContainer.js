...
import MiddleChckRegist02 from 'pages/usr/my/em/cr/middleChckRegist/comp/MiddleChckRegist02';
import SptEvlRgr05 from 'pages/usr/my/em/er/sptEvlRgr/comp/SptEvlRgr05';

const MiddleChckRegistContainer = ({ ojtKey, setPageType }) => {
  const [arrRjctInfo, setArrRjctInfo] = useState([]);
  const [arrAtchFile, setArrAtchFile] = useState([]);
  const [arrMiddleRptpAtchFile, setArrMiddleRptpAtchFile] = useState([]);
  ...
  return (
    <>
      ...
      <Row className="eu-usr-row-page-comp-layout">
        <Col span={24}>
          <MiddleChckRegist01 arrAtchFile={arrMiddleRptpAtchFile} />
        </Col>
      </Row>
...
export default React.memo(MiddleChckRegistContainer);
